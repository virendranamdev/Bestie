...
//                $forimage = "http://admin.benepik.com/employee/virendra/benepik_client/post_img/";
    ...
    function delete_Comment($com, $postId = '', $flag = '') {
        $this->commentid = $com;
		$status = 0;
		$sta = "Hide";
        try {
            if ($flag == 23) {		
				$query = "Update Tbl_C_FeedbackComments SET status = :status where commentId = :comm ";
                $query .= (!empty($postId)) ? "and feedbackId='$postId' " : "";
				$query .= (!empty($flag)) ? "and flagType='$flag' " : "";
            } 
			
			if ($flag == 16) { 			
				$query = "Update Tbl_Analytic_PostComment SET status = :status where commentId = :comm ";
                $query .= (!empty($postId)) ? "and postId='$postId' " : "";
				$query .= (!empty($flag)) ? "and flagType='$flag' " : "";
            } 
...
function delete_AlbumComment($commentid, $albumid , $imageid ,$flag) {
        ...
        try {
			...
			$query = "Update Tbl_Analytic_AlbumComment SET status = :status where commentId = :comm AND albumId = :albumid AND imageId = :imgid AND flag = :flag ";
			$stmt = $this->DB->prepare($query);
            $stmt->bindParam(':comm', $commentid, PDO::PARAM_STR);
			...
			{
				$response["success"] = 0;
                $response["message"] = "Comment not delete";
                return json_encode($response);
			}
			} 
        } catch (PDOException $e) {
				$response["success"] = 0;
                $response["message"] = "Comment not delete".$e;
				return json_encode($response);
        }
    }
