...
class ThoughtOfDay {
    ...
    function status_Post($thoughtid, $status) {
        $flag = 5;
        try {
            $query = "update Tbl_C_Thought set status = :sta where thoughtId = :thoughtid And flagType = :flag";
            $stmt = $this->DB->prepare($query);
            $stmt->bindParam(':thoughtid', $thoughtid, PDO::PARAM_STR);
            $stmt->bindParam(':sta', $status, PDO::PARAM_STR);
			$stmt->bindParam(':flag', $flag, PDO::PARAM_STR);
            $stmt->execute();

            $gquery = "update Tbl_Analytic_ThoughtSentToGroup set status = :sta2 where thoughtId = :thoughtid2 ";
            $stmtg = $this->DB->prepare($gquery);
            $stmtg->bindParam(':thoughtid2', $thoughtid, PDO::PARAM_STR);
            $stmtg->bindParam(':sta2', $status, PDO::PARAM_STR);
            $stmtg->execute();

            $wquery = "update Tbl_C_WelcomeDetails set status = :sta1 where id = :thoughtid1 And flagType = :flag1 ";
            $stmtw = $this->DB->prepare($wquery);
            $stmtw->bindParam(':thoughtid1', $thoughtid, PDO::PARAM_STR);
			...
			{
                $response["success"] = 1;
                $response["message"] = "Thought status has changed";
            }
			else
			{
				$response["success"] = 0;
                $response["message"] = "Thought status not change";
			}
        } catch (PDOException $e) {
            $response["success"] = 0;
            $response["message"] = "Thought status not change".$e;
        }
		return json_encode($response);
    }
