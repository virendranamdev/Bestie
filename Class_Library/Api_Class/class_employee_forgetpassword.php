            ...
            if ($stmt->execute()) {
                ...
                if ($result) {
                    if ($result["responseDecider"] == 3 && !empty($result["emailId"])) {
                        ...
                        $query = "update Tbl_EmployeeDetails_Master set password=:pass where clientId=:cli and employeeId=:emi and ((UPPER(employeeCode)=:empcode) or (UPPER(emailId)=:empcode))";
                        $stmt = $this->db_connect->prepare($query);
                        ...
                        $response = array();
                        $response["success"] = 0;
                        $response["message"] = "Please register with Bestie first. ";
                    }
                } else {
                    $response = array();
                    $response["success"] = 0;
