  ...
  function AchiverStoryDisplay($clientid,$uid,$val)
     ...
     $this->value = $val;
       ...
       // print_r($value);
            ...
            /****************************************************************************************************/
			    ...
			    $eventquery1 = "select count(distinct(storyId)) as total from Tbl_Analytic_StorySentToGroup where groupId IN('".$in."') and clientId =:cid AND status = 1 order by autoId desc";
			...
			/**********************************************************************************/  
				  $eventquery = "select distinct(storyId) from Tbl_Analytic_StorySentToGroup where groupId IN('".$in."') and clientId =:cid And status = 1 order by autoId desc limit $val, 5";
	...
	/********************************************************************/
						...
						for($w=0;$w<$welcount;$w++)
						{
							 ...
							 $query2 = "select * , if(imagePath IS NULL OR imagePath = '' , '' , concat('".site_url."',imagePath)) as imagePath , DATE_FORMAT(createdDate,'%d %b %Y') as createdDate from Tbl_C_AchiverStory where storyId=:pstid and clientId=:cli and status = 1 order by storyId";
							...
							$post["achieverName"]=$rows2["achieverName"];
							$post["designation"]=$rows2["designation"];
							$post["location"]=$rows2["location"];
							...
							$queryi = "select if(imagePath IS NULL OR imagePath = '' , '' , concat('".site_url."',imagePath)) as imagePath from Tbl_C_AchieverImage where storyId=:ipstid and clientId=:icli";
							 $stmti = $this->DB->prepare($queryi);
							 $stmti->bindParam(':icli',$this->idclient, PDO::PARAM_STR);
							 $stmti->bindParam(':ipstid',$postid, PDO::PARAM_STR);
							 $stmti->execute();
							 $rowsi = $stmti->fetch(PDO::FETCH_ASSOC);
							 
							 //print_r($rowsi);
							$post["imagePath"]=$rowsi["imagePath"];
							
							 $query = "select Tbl_EmployeeDetails_Master.firstName, Tbl_EmployeeDetails_Master.middleName,Tbl_EmployeeDetails_Master.lastName,if(Tbl_EmployeePersonalDetails.userImage IS NULL or Tbl_EmployeePersonalDetails.userImage='', '', if(Tbl_EmployeePersonalDetails.linkedIn = '1', Tbl_EmployeePersonalDetails.userImage, Concat('". site_url ."',Tbl_EmployeePersonalDetails.userImage))) as UserImage 
							...
							$post["UserName"]=$rows["firstName"]." ".$rows["middleName"]." ".$rows["lastName"];
    ...
    /************************************************************************************************/
        ...
        catch(PDOException $e)
	    {
		   echo $e;
		   $result['success'] = 0;
		   $result['message'] = "No more post available".$e;
		   return $result;
	    }
    }
	
/************************************ get single achiever detail *******************/
 public function achiever_details($clientid, $storyId, $flag) {
        try {
			...
			$post["storyId"] = $rows["storyId"];
            $post["clientId"] = $rows["clientId"];
            $post["title"] = $rows["title"];
			$post["achieverName"] = $rows["achieverName"];
			$post["designation"] = $rows["designation"];
			$post["location"] = $rows["location"];
			$post["story"]=$rows["story"];
			$post["device"] = $rows["device"];
			$post["status"] = $rows["status"];
			$post["thumb_imagePath"] = $rows["thumb_imagePath"];
			$post["flagType"] = $rows["flagType"];
			$post["likeType"] = $rows["likeType"];
			$post["comment"] = $rows["comment"];
			$post["createdBy"] = $rows["createdBy"];
			$post["createdDate"] = $rows["createdDate"];
			$post["updatedDate"] = $rows["updatedDate"];
			$post["updatedBy"] = $rows["updatedBy"];
			$post["userImage"] = $rows["userImage"];
			
			$query1 = "select imageId, if(imagePath IS NULL OR imagePath = '', '' , CONCAT('".$site_url."',imagePath)) as imagePath , status from Tbl_C_AchieverImage where clientId=:cli1 and storyId=:storyId1";
			...
			foreach ($rows1 as $row) {
                    ...
                    $post2["imageId"] = $row["imageId"];
                    $post2["imagePath"] = $row["imagePath"];
					$post2["status"] = $row["status"];
                    array_push($img, $post2);
                }
			
			//print_r($img);
			
			$post["image"] = (!empty($img))?$img:"";
