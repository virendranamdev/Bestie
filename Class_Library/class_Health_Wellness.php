...
class HealthWellness {
    ...
    public function createHealthExercise($clientId, $exercise_area_id, $exercise_name, $exercise_image, $exercise_instructions, $status, $create_Date, $flag) {
        try {
            $query = "insert into Tbl_HealthWellness(clientId,exercise_area_id,exercise_name,exercise_image,exercise_instruction,flagType,status,create_date)
            values(:cid,:exercise_type,:exercise_name,:exercise_image,:instruction,:flag,:status,:create_date)";
    ...
    public function updateHealthExercise($exercise_id, $exercise_area_id, $exercise_name, $exercise_image, $exercise_instructions, $update_Date, $flag) {
        try {
            $query = "UPDATE Tbl_HealthWellness SET exercise_area_id=:exercise_type, exercise_name=:exercise_name, exercise_image=:exercise_image, exercise_instruction=:instruction, flagType=:flag, update_date=:update_date WHERE autoId=:exerciseId";
            $stmt = $this->DB->prepare($query);
...
//        }
    ...
    public function getExercises($clientId, $device = '') {
        try {
            ...
            $query = "SELECT health.*, if(health.exercise_image IS NULL or health.exercise_image='', '', CONCAT('" . $site_url . "',health.exercise_image)) as exercise_image, exercise.exercise_area,exercise.exercise_area FROM Tbl_HealthWellness AS health JOIN Tbl_Exercise as exercise ON health.exercise_area_id=exercise.autoId WHERE health.clientId=:cid";
            $stmt = $this->DB->prepare($query);
            $stmt->bindParam(':cid', $clientId, PDO::PARAM_STR);
            if ($stmt->execute()) {
                $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
